...
public class ViewQueryParameters {
    ...
    public ViewQueryParameters(List<ViewFilter> filters) {
        ...
        Optional<ViewFilter> parentNameFilter = filters.stream()
                ...
                .findFirst();
        parentViewNamePattern =
                parentNameFilter.isPresent() ? parentNameFilter.get().getPrefix() : null;

        List<ViewFilter> attributeFilters = filters.stream()
                .filter(f -> !f.getAttribute().equals("viewName"))
                .collect(Collectors.toList());
        filterMap = attributeFilters.stream().collect(Collectors.groupingBy(filter -> {
        ...
        if (filterMap.containsKey(AttributeType.UNCLASSIFIED.toString())) {
            ...
            throw new IllegalArgumentException(
                    "ViewFilter could not be classified as one of " + AttributeType.values() + ". Filters: "
                            + unclassified.stream().map(ViewFilter::toString).collect(Collectors.joining("\n")));
        }
    }
